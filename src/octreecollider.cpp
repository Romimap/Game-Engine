...
RayCastHit OctreeCollider::GridTreeIntersect (QVector3D O, QVector3D D, QVector3D gridPos, QVector3D offset, QVector3D gridSize, int layer) {
        ...
        if (layer <= -1) {
            offset /= SCALEFACTOR;
            if (_octreeComponent->getVoxelType(offset.x(), offset.y(), offset.z(), layer + 1) != MaterialId::AIR) {
                hit._distance = 0;
            }
            return hit;
        }
        ...
        bool isFull = _octreeComponent->getVoxelType(X + offset.x(), Y + offset.y(), Z + offset.z(), layer) != MaterialId::AIR;
        if (isFull) {
            hit._distance = 0;
        ...
        while (true) {
            if (tmaxx < tmaxy && tmaxx < tmaxz) {
                X += Xstep;
                if (X >= 0 && X < gridSize.x()) {
                    isFull = _octreeComponent->getVoxelType(X + offset.x(), Y + offset.y(), Z + offset.z(), layer) != MaterialId::AIR;
                    if (isFull) {
